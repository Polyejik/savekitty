(()=>{
  ...
  function callNative(){const fn=window.openHonor||window.showHonor||window.openLeaderboard||window.showLeaderboard;if(typeof fn==='function'){try{fn();return true}catch{}}const candidates=[...document.querySelectorAll('[id],[class]')].filter(el=>el.id!=='sk-honor-fallback'&&/honor|leader|hall/i.test((el.id||'')+' '+(el.className||'')));for(const el of candidates){if(el instanceof HTMLElement&&getComputedStyle(el).display==='none'){el.style.display='block';return true}}return false}
  function ensure(){addStyle();const locks=q('#locks');if(!locks)return;let shell=locks.closest('.locksMeta');if(!shell){shell=document.createElement('div');shell.className='locksMeta';locks.parentNode.insertBefore(shell,locks);shell.appendChild(locks)}let btn=q('#honorBtn');if(!btn){btn=document.createElement('button');btn.id='honorBtn';btn.type='button';btn.className='honorBtn';btn.textContent='🏆';btn.title='Доска почёта / Hall of Fame';btn.setAttribute('aria-label','Доска почёта');btn.onclick=()=>{if(!callNative())openFallback()}}btn.classList.add('sk-restored');if(!btn.textContent.trim())btn.textContent='🏆';if(btn.parentNode!==shell)shell.appendChild(btn)}
  const start=()=>{ensure();setInterval(ensure,500)};
