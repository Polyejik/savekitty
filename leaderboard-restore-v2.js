(()=>{
  ...
  const css=`
    #honorBtn.sk-restored{min-width:46px!important;width:46px!important;height:42px!important;padding:0!important;margin:0!important;border:1px solid #76543a!important;border-radius:12px!important;background:linear-gradient(#463328,#2b201a)!important;color:#ffe4a3!important;font:900 19px/1 system-ui!important;display:flex!important;align-items:center!important;justify-content:center!important;box-shadow:0 2px 6px #0004!important}
  ...
  function nativeTarget(btn){return QA('button,a,[role="button"]').find(el=>el!==btn&&/доска поч[её]та|hall of fame|поч[её]т/i.test((el.textContent||'')+' '+(el.title||'')+' '+(el.getAttribute('aria-label')||'')))}
  function ensure(){addStyle();const locks=q('#locks');if(!locks)return;let btn=q('#honorBtn');if(btn){btn.style.removeProperty('display');btn.style.removeProperty('visibility');btn.style.removeProperty('opacity');btn.classList.add('sk-restored');return}
    btn=document.createElement('button');btn.id='honorBtn';btn.type='button';btn.className='honorBtn sk-restored';btn.textContent='🏆';btn.title='Доска почёта / Hall of Fame';btn.setAttribute('aria-label','Доска почёта');btn.onclick=()=>{const t=nativeTarget(btn);if(t){t.click();return}const fn=window.openHonor||window.showHonor||window.openLeaderboard||window.showLeaderboard;if(typeof fn==='function'){try{fn();return}catch{}}openFallback()};
    const shell=locks.closest('.locksMeta')||locks.parentElement;shell.appendChild(btn)
  }
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>{ensure();setInterval(ensure,800)});else{ensure();setInterval(ensure,800)}
})();
