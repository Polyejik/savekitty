...
function findStart(i){const start=document.getElementById('nameStart');if(start)return start;let root=i?.closest('.nameGate')||i?.parentElement?.parentElement||document;let buttons=[...root.querySelectorAll('button')];return buttons.find(b=>/начать|играть|start|play/i.test(b.textContent||''))}
