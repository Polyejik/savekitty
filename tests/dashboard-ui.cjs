...
const response={ok:true,summary:{confirmed_rescues:9,donation_rub:90,budget_rub:500000,unique_players:5,today_runs:3,avg_seconds:197,repeat_rate:20,raw_runs:67,duplicates:57,too_fast:1,rate_rub:10,days_left:29},daily:Array.from({length:14},(_,i)=>({day:`2026-09-${String(i+1).padStart(2,'0')}`,value:i===13?9:0})),runs:[{completed_at:'2026-09-14T10:00:00Z',participant:'<img src=x onerror=alert(1)>',duration_seconds:197,game_version:'2026.09.13.2',status:'confirmed'}]};
...
(async()=>{
 await wait();assert.equal(q('state').textContent,'LIVE');assert.equal(q('confirmed').textContent,'9');assert.equal(q('avg').textContent,'3:17');
 ...
 response.runs=[];response.daily.forEach(x=>x.value=0);response.summary.confirmed_rescues=0;response.summary.donation_rub=0;
 q('refresh').click();await wait();assert.equal(q('confirmed').textContent,'0');assert.match(q('runs').textContent,/первое подтверждённое/);assert.equal(q('budgetFill').style.width,'0%');
